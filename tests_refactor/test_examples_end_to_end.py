...
def test_examples_minimal_end_to_end_like_simple_demo():
    ...
    res = compile_ir(circ, compile_engine="default", output="ir", device_rule={"max_shots_per_job": 16}, options={"total_shots": 33})
    plan = res["metadata"].get("execution_plan") or schedule(res["circuit"], total_shots=33, device_rule={"max_shots_per_job": 16})
