...
def test_dep(backend):
    ...
    cs = tq.channels.generaldepolarizingchannel([0.1, 0.1, 0.1], 1)
    tq.channels.kraus_identity_check(cs)

    cs = tq.channels.generaldepolarizingchannel(0.02, 2)
    tq.channels.kraus_identity_check(cs)

    cs2 = tq.channels.isotropicdepolarizingchannel(0.02 * 15, 2)
...
def test_rep_transformation(backend):
    ...
    kraus_set.append(tq.channels.resetchannel())
    kraus_set.append(tq.channels.generaldepolarizingchannel(0.1, 1))
    kraus_set.append(tq.channels.phasedampingchannel(0.5))
    ...
    for density_matrix in density_set:
        for kraus in kraus_set:
            tq.channels.check_rep_transformation(kraus, density_matrix, verbose=False)

    kraus = tq.channels.generaldepolarizingchannel(0.01, 2)
    ...
    # test
    choi = np.zeros([4, 4])
    kraus = tq.channels.choi_to_kraus(choi)
...
def test_thermal(backend):
    ...
    t1 = 180
    kraus = tq.channels.thermalrelaxationchannel(t1, t2, time, "AUTO", 0.1)
    supop1 = tq.channels.kraus_to_super(kraus)

    kraus = tq.channels.thermalrelaxationchannel(t1, t2, time, "ByKraus", 0.1)
    supop2 = tq.channels.kraus_to_super(kraus)
    ...
    kraus = tq.channels.thermalrelaxationchannel(t1, t2, time, "ByChoi", 0.1)
    supop2 = tq.channels.kraus_to_super(kraus)
...
def test_noisecircuit(backend):
    ...
    def noisecircuit(X):
        ...
        c = tq.Circuit(n)
        c.x(0)
        # noise = tq.channels.thermalrelaxationchannel(300, 400, 1000, "AUTO", 0)
    ...
    noisec_vmap = tq.backend.vmap(noisecircuit, vectorized_argnums=0)
    noisec_jit = tq.backend.jit(noisec_vmap)

    nmc = 10000
    X = tq.backend.implicit_randu(nmc)
    valuemc = sum(tq.backend.numpy(noisec_jit(X))) / nmc
    ...
    noisec_jit = tq.backend.jit(noisecircuitdm)
    valuedm = noisec_jit()
...
def test_readout(backend):
    ...
    readout_error = tq.array_to_tensor(readout_error)
    value = c.sample_expectation_ps(z=[0, 1, 2], readout_error=readout_error)
    valueaim = 0.04
    np.testing.assert_allclose(value, valueaim, atol=1e-1)
    ...
    calvalue = tq.backend.jit(jitest)
...
def test_noisesample(backend):
    ...
    calsample = tq.backend.jit(jitest)
