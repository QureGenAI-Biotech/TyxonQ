...
def gate_list(param):
    l = [
        ...
        tq.gates.rx_gate.ided(before=False)(theta=param),
        tq.gates.ry_gate.ided(before=False)(theta=param),
        tq.gates.rz_gate.ided(before=False)(theta=param),
...
def vqef(param, structure):
    ...
    e = tq.templates.measurements.operator_expectation(c, h)
    return e


# warning pytorch might be unable to do this
vqegf = tq.backend.vmap(
    tq.backend.vvag(vqef, argnums=0, vectorized_argnums=0),
