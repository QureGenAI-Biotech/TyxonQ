...
| キーワード | 機能説明 | 構文形式 |
|------------|----------|----------|
| `defcal` | カスタムパラメータ付き波形量子ゲートの定義 | `defcal <ゲート名> <パラメータリスト> { <キャリブレーション文> }` |
...
以下は、パラメータ付き波形の定義と使用方法を示す完全なTQASM 0.2コード例です：
...
TyxonQパルスインターフェースの主な構成要素には、波形の種類、パラメータ指定機能、およびキャリブレーションビルダーが含まれます。これらのコンポーネントが連携することで、ユーザーはパルスの挙動を柔軟かつ詳細に制御できます。
---

### 1. 波形種類

TyxonQは、特定のパラメータを持つ複数の事前定義されたパルス波形をサポートします：
...
すべての波形は、`Param` クラスを用いたパラメータ指定に対応しています：
...
TyxonQのパルスインターフェースは、強力なマイクロ波パルスの制御機能を提供しています。波形の適切な選択、パラメータ指定、およびキャリブレーションビルダーの活用によって、複雑な量子制御実験を柔軟に実現することが可能です。はじめはシンプルな例から始め、徐々に応用範囲を広げていくことをおすすめします。
